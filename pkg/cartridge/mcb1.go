...
func (mbc1 *MBC1) Read(addr uint16) uint32 {
	if addr <= LOWER_ROM_BANK_END {
		return uint32(addr) + uint32(mbc1.getLowerRomBank())*0x4000
	} else if addr >= UPPER_ROM_BANK_START && addr <= UPPER_ROM_BANK_END {
		return uint32(addr-0x4000) + uint32(mbc1.getUpperRomBank())*0x4000
	} else if mbc1.hasRam && mbc1.ramEnabled && addr >= RAM_BANK_START && addr <= RAM_BANK_END {
		return uint32(addr) + uint32(mbc1.getRamBank())*0x2000
	}

	return 0
}
func (mbc1 *MBC1) Write(addr uint16, data byte) uint32 {
